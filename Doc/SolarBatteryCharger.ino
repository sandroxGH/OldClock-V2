//------------------------------------------------------------------------------------------------------
//
// Arduino 12V Solar Battery Charger    
//
//  This code is a modified version of sample code from   http://www.timnolan.com/.
// It has been modified to fit our hardware and project reqirements.
//
//------------------------------------------------------------------------------------------------------
...
#define MIN_BAT_VOLTS 800          //value of battery voltage scaled by 100 so this is 11.00 volts          
...
void read_data(void) {
  ...
  sol_amps =  ((read_adc(SOL_AMPS_CHAN)  * SOL_AMPS_SCALE) + 5) / 10;    //input of solar amps
  sol_volts = ((read_adc(SOL_VOLTS_CHAN) * SOL_VOLTS_SCALE) + 5) / 10;   //input of solar volts 
  bat_volts = ((read_adc(BAT_VOLTS_CHAN) * BAT_VOLTS_SCALE) + 5) / 10;   //input of battery volts 
  sol_watts = (int)((((long)sol_amps * (long)sol_volts) + 50) / 100);    //calculations of solar watts                  
}
